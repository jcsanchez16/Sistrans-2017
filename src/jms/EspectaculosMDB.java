...
public class EspectaculosMDB implements MessageListener, ExceptionListener 
{
	public final static int TIME_OUT = 5;
	private final static String APP = "app3";
	
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicEspectaculoss";
	...
	private List<Espectaculo> answer = new ArrayList<Espectaculo>();
	
	public EspectaculosMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
...
//		id = new String(md.digest(id.getBytes()));
		...
		ListaEspectaculo res = new ListaEspectaculo(answer);
        return res;
	}
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("espectaculos.general.app3", APP, payload, status, id);
	...
	public void onMessage(Message message) 
	{
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				...
				{
					FestAndesDistributed dtm = FestAndesDistributed.getInstance();
					ListaEspectaculo espectaculos = dtm.getLocalEspectaculos();
					String payload = mapper.writeValueAsString(espectaculos);
					Topic t = new RMQDestination("", "espectaculos.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ListaEspectaculo e = mapper.readValue(ex.getPayload(), ListaEspectaculo.class);
					answer.addAll(e.getLista());
				}
			}
